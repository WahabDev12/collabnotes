...
class MainLogic(tk.Frame):
    ...
    def create_widgets(self):
        ...
        # Caption
        self.main_caption = tk.Label(self.master,fg="#0D2E28",bg="#10C5A7", text='Collaborative Notes App', font=('Comic Sans MC', 25,"italic"))
        self.main_caption.place(x = 200, y = 15)

        # Title
        self.title = tk.StringVar()
        self.title_caption = tk.Label(
            self.master, fg="#0D2E28",bg="#10C5A7", text='Title of Note', font=('bold', 14), pady=20)
        self.title_caption.place(x = 300, y = 50)
        self.title_entry = tk.Entry(self.master, textvariable=self.title,font=('bold', 14))
        self.title_entry.place(x = 190, y = 110, width=400,height = 30)

        # Description
        self.description= tk.StringVar()
        self.description_label = tk.Label(
            self.master,fg="#0D2E28",bg="#10C5A7", text='Note Description (150 words max)', font=('bold', 14))
        self.description_label.place(x = 250, y = 160)
        self.description_entry = tk.Entry(
            self.master, textvariable=self.description,font=('bold', 11))
        self.description_entry.place(x = 190, y = 200,width=400, height=90)

        # Buttons
        self.add_btn = tk.Button(
            self.master, text="Add Note", width=12, command=self.add_note)
        self.add_btn.place(x = 150, y = 350)

        self.delete_btn = tk.Button(
            self.master, text="Edit Note", width=12, command=self.update_note)
        self.delete_btn.place(x=290,y=350)

        self.update_btn = tk.Button(
            self.master, text="Delete Note", width=12, command=self.delete_note)
        self.update_btn.place(x=420,y=350)

        self.exit_btn = tk.Button(
            self.master, text="Reset", width=12, command=self.clear_notes)
        self.exit_btn.place(x=560,y=350)


        # List of Notes Table
        self.notes_list = tk.Listbox(self.master,bg = "#293230",fg="white", height=10, width=100, border=0,borderwidth=0, highlightthickness=0, font = "Times 10")
        ...
        # Instruction
        self.instruction = tk.Label(self.master,fg="#0D2E28",bg="#10C5A7", text='* Select a note to edit or delete *', font=('Comic Sans MC', 15,"italic"))
